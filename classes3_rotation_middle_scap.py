...
class SCHR:
    ...
    def add_muscle(self,l0=1,koef=5,epsm0=0.5,F_iso=1,thorax_ins=None,scap_ins=None,humer_ins=None,GHjoint=None,muscle_group='ThorScap',muscle_model='Thelen'):
        if muscle_group=='ThorScap':
            ...
            B = R_phi(self.phis)*T_x(GHjoint[0])*T_y(GHjoint[1])*R_phi(self.alfa-self.phis)*position(humer_ins[0],humer_ins[1])
            length = sp.sqrt((B[0]-A[0])**2+(B[1]-A[1])**2)
            if muscle_model=='Thelen':
                F,U = self.thelen_03_muscle_pas_force(l0,koef,epsm0,F_iso)
            elif muscle_model=='Mclean':
